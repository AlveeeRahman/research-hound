# Schematics (Stage 3 — make the idea visible)
...
python scripts/schematics/generate_schematic.py "CONSORT participant flow diagram with 500 screened, 150 excluded, 350 randomized" -o figures/consort.png --doc-type journal

# Generate for presentation (lower threshold: 6.5/10 - faster)
python scripts/schematics/generate_schematic.py "Transformer encoder-decoder architecture showing multi-head attention" -o figures/transformer.png --doc-type presentation

# Generate for poster (moderate threshold: 7.0/10)
python scripts/schematics/generate_schematic.py "MAPK signaling pathway from EGFR to gene transcription" -o figures/mapk_pathway.png --doc-type poster

# Custom max iterations (max 2)
python scripts/schematics/generate_schematic.py "Complex circuit diagram with op-amp, resistors, and capacitors" -o figures/circuit.png --iterations 2 --doc-type journal
```
...
```bash
python scripts/schematics/generate_schematic.py "your diagram description" -o output.png
```
...
[references/schematics/iterative_refinement.md](references/schematics/iterative_refinement.md).
...
```bash
# Basic usage
python scripts/schematics/generate_schematic.py "diagram description" -o output.png

# Custom iterations (max 2)
python scripts/schematics/generate_schematic.py "complex diagram" -o diagram.png --iterations 2

# Verbose mode
python scripts/schematics/generate_schematic.py "diagram" -o out.png -v
```
...
- **`references/schematics/iterative_refinement.md`** - The generate-review-refine loop, the Python API, every
  command-line option, prompt engineering guidance, and four worked examples
- **`references/schematics/best_practices.md`** - Publication standards and accessibility guidelines to draw
